            ...
            .widget(
                    'ui.tagger',
                    {
                        options : {
                            ...
                            ajaxErrorFunction : function(self, data) {
                                self._showMessageSuggestion('AJAX Search failed', 'error');
                            },
                        ...
                        _create : function() {
                            ...
                            if (this.element.is('select')) {
                                ...
                                this.taggerWidget = $('<div>').addClass('tagger form-control input-sm').insertAfter(this.element);
                                if (this.readonly) {
                                ...
                                if (!this.readonly) {
                                    ...
                                    this.taggerInput = $('<input>').attr('type', 'text').attr('autocomplete', 'off').addClass('intxt').appendTo(
                                            this.taggerWidget);
                                    this.taggerButtonsPanel = $('<div>').addClass('tagger-buttons');
                                    this.taggerButtonsPanel.appendTo(this.taggerWidget);

                                    if (!this.options.ajaxURL) {
                                        this.taggerSuggestionsButton = $('<div>').addClass('droparrow').addClass('hittarget').bind('mouseup keyup',
                                                $.proxy(this._handleSuggestionsButtonInteraction, this)).appendTo(this.taggerButtonsPanel);
                                        $('<img>').attr('src', this.options.baseURL + this.options.imgDownArrow).appendTo(
                                                this.taggerSuggestionsButton);
                                    } else {
                                        this.taggerSuggestionsButton = $('<div>').addClass('search').bind('mouseup keyup',
                                                $.proxy(this._handleSuggestionsButtonInteraction, this)).appendTo(this.taggerButtonsPanel);
                                        $('<img>').attr('src', this.options.baseURL + this.options.imgSearch).attr('title',
                                                this.options.searchTooltipText).appendTo(this.taggerSuggestionsButton);
                                    }
                                ...
                                $('<div>').addClass('clearer').appendTo(this.taggerWidget);

                                if (!this.readonly) {
                                    // If not readonly, stub out an empty suggestion list
                                    this.taggerSuggestions = $('<div>').addClass('suggestions').appendTo(this.taggerWidget);

                                    // Put a filter at the top of the suggestion list in single-select mode
                                    if (this.singleValue) {
                                        this.taggerFilterInput = $('<input type="text" class="filtertxt" autocomplete="off"/>').appendTo(
                        ...
                        _handleTaggerKeypressRedirect : function(event) {
                            if (event.which !== 0 && event.charCode !== 0 && !event.ctrlKey && !event.metaKey && !event.altKey) {
                                ...
                                // typed
                                if (event.target === this.taggerInput.get(0)
                                        || (this.taggerFilterInput && event.target === this.taggerFilterInput.get(0))) {
                                    return;
                                }
                            ...
                            for ( var tagID in this.tagsByID) {
                                if (this.tagsByID.hasOwnProperty(tagID)) {
                                    ...
                                    if (this.options.caseSensitive) {
                                        if (tag.key.indexOf(searchString) >= 0 || (tag.hidden && tag.hidden.indexOf(searchString) >= 0)
                                        ...
                                        if (tag.key.toLowerCase().indexOf(searchStringLowerCase) >= 0
                                                ...
                                                || $('<div/>').html(tag.suggestion).text().replace(/<.*?[^>]>/g, '').toLowerCase().indexOf(
                                                        searchStringLowerCase) >= 0) {
                                            filteredResults[tagID] = $.extend(true, {}, tag);
                                            filteredResults[tagID].suggestable = true;
                                        }
                                    }
                        ...
                        _ajaxLoadSuggestions : function(value) {
                            ...
                            this.pendingFilterEvent = setTimeout(function() {
                                $.ajax({
                        ...
                        _loadSuggestions : function(suggestableTags, allowIndent) {
                            ...
                            if (this.options.sortedOutput) {
                                ...
                                suggestableTagArray.sort(function(a, b) {
                                    if (a[1] === undefined) {
                                        return b[1];
                            ...
                            for (var i = 0; i < suggestableTagArray.length; i++) {
                                ...
                                if ((!tag.suggestable && !this.options.displayHierarchy) || tag.historical || !(tag.key && tag.id)
                                        || (!this.options.freeTextSuggest && tag.freetext)) {
                                    continue;
                                }
                            ...
                            if (this.options.freeTextInput && this._getVisibleInput().val().length > 0) {
                                ...
                                if (this.options.freeTextMessage) {
                                    message = this.options.freeTextMessage.replace(/%VALUE%/g, $("<div>").text($.trim(this._getVisibleInput().val()))
                                            .html());
                                } else {
                                    message = this._getVisibleInput().val();
                                }

                                $('<li>').addClass('extra').addClass('addfreetext').attr("tabindex", this.tabIndex).html(message).data("freetext",
                                        this._getVisibleInput().val()).bind('mouseup keyup keydown',
                                        $.proxy(this._handleSuggestionItemInteraction, this)).bind('mouseleave mouseenter blur focus',
                                        $.proxy(this._handleSuggestionItemFocus, this)).appendTo(this.taggerSuggestionsList);
                            }

                            if (suggestableTagArray.length === 0) {
                                // Add message if filtering meant no items to suggest
                                $('<li>').addClass('extra').addClass('missing').text(this.options.noSuggestText).appendTo(this.taggerSuggestionsList);
                            }
                            ...
                            if (this.taggerSuggestionsList.children().length === 0) {
                                $('<li>').addClass('extra').addClass('missing').text(this.options.emptyListText).appendTo(this.taggerSuggestionsList);
                            }

                            if (suggestableTags.limited) {
                                $('<li>').addClass('extra').addClass('limited').text(this.options.limitedText).appendTo(this.taggerSuggestionsList);
                            }
                        },
                        ...
                        _createSuggestionsItem : function(tag, allowIndent) {
                            // Create and add the suggestion to the suggestion list
                            var suggestion = $('<li>').attr("suggestion", "tag").appendTo(this.taggerSuggestionsList);
                        ...
                        _handleSuggestionItemInteraction : function suggestionBind(event) {
                            if (event.type !== "mouseup" && event.type !== "keyup" && event.type !== "keydown") {
                                throw "Unhandled event type passed to _handleSuggestionItemInteraction(), expected mouseup, keyup or keydown): "
                                        + event.type;
                            }
                            ...
                            if ((event.type === "mouseup" && event.which === this.mouseCodes.LEFT)
                                ...
                                if (suggestionItem.data('tagid') && !suggestionItem.data('freetext')) {
                                   /* if (this.options.maxTagNumber > 0 && this.options.maxTagNumber <= this.tagCount) {
                                        alert("You can select Max " + this.options.maxTagNumber + " options");
                                        this._selectionReset(true, true);
                                        return;
                                    }*/
                                ...
                                event.preventDefault();
                                    ...
                                    && (event.which === this.keyCodes.UP || (event.which === this.keyCodes.TAB && event.shiftKey))) { // Up arrow / shift+tab (Move selection up and up into the input)
                                ...
                                event.preventDefault();
                                    ...
                                    && (event.which === this.keyCodes.DOWN || (event.which === this.keyCodes.TAB && !event.shiftKey))) { // Down arrow / tab (Move selection down, stop at the end)
                                // Override default browser tab control and allow arrow keys too
                        ...
                        _showSuggestions : function(focusFirstItem) {
                            ...
                            if (this.taggerSuggestionsList.children().length === 0) {
                                ...
                                if ($.map(this.tagsByID, function(n, i) {
                                    return i;
                                }).length > 300) {
                                    $('<li>').addClass('extra').addClass('missing').text('Loading...').appendTo(this.taggerSuggestionsList);
                        ...
                        _inputExpand : function(input) {
                            ...
                            if (input.val() !== "") {
                                ...
                                taggerInputSpan.html(input.attr('placeholder').replace(/&/g, '&amp;').replace(/\s/g, '&nbsp;').replace(/</g, '&lt;')
                                        .replace(/>/g, '&gt;'));
                            }

                            // Set the width of the input to be the width of the span, making sure not to overflow the general widget bounds
                            input.width(Math.min(input.parent().innerWidth() - this.taggerSuggestionsButton.outerWidth(), taggerInputSpan.width()
                                    + this.options.inputExpandExtra));
                        ...
                        _addTagFromID : function(tagID) {
                            ...
                            var nextPaydayId = this.element[0].id;
                           ...
                           // if (this.singleValue) {
                            ...
                            if (!this.readonly) {
                                ...
                                if ($('option[value="' + tagID.replace(/"/g, '\\"') + '"]', this.element).length > 0) {
                                    ...
                                    $('<option>').prop("selected", true).val(tagID).text($('<div>').html(tagData.key).text()).appendTo(this.element);
                                }
                                // Add the HTML to show the tag
                                tag = $('<div>').addClass('tag').attr("tabindex", this.tabIndex).text($('<div/>').html(tagData.key).text()).data(
                                        "tagid", tagID).insertBefore(this.taggerInput);

                                if (tagData.freetext) {
                                    tag.addClass('freetext');
                                }

                                var tagRemover = $('<span class="removetag hittarget" ><img src="' + this.options.baseURL + this.options.imgRemove
                                        + '" /></span>');
